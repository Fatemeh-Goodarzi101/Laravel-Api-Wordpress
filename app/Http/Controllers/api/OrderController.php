...
class OrderController extends Controller
{
    ...
    public function sendOrderReport(Request $request)
    {
        ...
        if (!is_null($request->input('orderDateFrom'))) {
            $reportOrderDateFrom = DB::table('wp_wc_order_product_lookup')->whereDate('date_created', '>=', $request->input('orderDateFrom'))->get(['order_item_id', 'order_id', 'product_id'])->toArray();
        ...
        if (!is_null($request->input('orderDateTo'))) {
            $reportOrderDateTo = DB::table('wp_wc_order_product_lookup')->whereDate('date_created', '<=', $request->input('orderDateTo'))->get(['order_item_id', 'order_id', 'product_id'])->toArray();
        ...
        if (!is_null($request->input('orderDateTo')) && !is_null($request->input('orderDateFrom'))) {
            $reportOrderDateBetween = DB::table('wp_wc_order_product_lookup')->whereBetween('date_created', [$request->input('orderDateFrom'), $request->input('orderDateTo')])->get(['order_item_id', 'order_id', 'product_id'])->toArray();
        ...
        foreach ($finalOrderId as $item) {
            $data = DB::select(DB::raw("SELECT items.order_id,items.order_item_id,items.order_item_name,
                    ...
                    JOIN wp_wc_order_stats AS status ON items.order_id = status.order_id
                    JOIN wp_wc_customer_lookup AS customer ON status.customer_id = customer.customer_id
    ...
    {
         (new OrderExport($finalOrders))->queue('export.xlsx');
         return back()->withSuccess('export started');
//        return Excel::download(new OrderExport($finalOrders), 'export.xlsx');
    }
